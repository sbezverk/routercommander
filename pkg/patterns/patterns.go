...
var LC = regexp.MustCompile(`\d+\/\d+\/CPU0\s+[a-zA-Z0-9\-\(\)]+\s+IOS XR RUN\s+`)

var RP = regexp.MustCompile(`\d+\/(RS?P){1}\d+\/CPU0\s+[a-zA-Z0-9\-\(\)]+\s+IOS XR RUN\s+`)

var ActiveRP = regexp.MustCompile(`[Aa]ctive`)

var Prompt = regexp.MustCompile(`(?m)RP\/\d\/(RS?P)?\d\/CPU[0-9]:[0-9A-Za-z-\.\_]+(\([0-9A-Za-z-\.\_]+\))?#(\n|$)`)

var SysadminPrompt = regexp.MustCompile(`(?m)sysadmin-vm:[0-1]_RP[0-1]#(\n|$|\s*)`)
